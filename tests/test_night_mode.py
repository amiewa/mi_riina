...
class TestNightMode:
    ...
    def test_night_time_23(self, mock_dt) -> None:
        ...
        mock_dt.now.return_value = datetime(2026, 3, 6, 23, 0, tzinfo=ZoneInfo("Asia/Tokyo"))
    ...
    def test_night_time_0(self, mock_dt) -> None:
        """0:00 は夜間（日跨ぎ）"""
        from zoneinfo import ZoneInfo
        mock_dt.now.return_value = datetime(2026, 3, 6, 0, 0, tzinfo=ZoneInfo("Asia/Tokyo"))
        assert is_night_mode(23, 5) is True
    ...
    def test_night_time_459(self, mock_dt) -> None:
        """4:59 は夜間"""
        from zoneinfo import ZoneInfo
        mock_dt.now.return_value = datetime(2026, 3, 6, 4, 59, tzinfo=ZoneInfo("Asia/Tokyo"))
        assert is_night_mode(23, 5) is True
    ...
    def test_day_time_5(self, mock_dt) -> None:
        """5:00 は昼間（夜間終了）"""
        from zoneinfo import ZoneInfo
        mock_dt.now.return_value = datetime(2026, 3, 6, 5, 0, tzinfo=ZoneInfo("Asia/Tokyo"))
    ...
    def test_day_time_12(self, mock_dt) -> None:
        """12:00 は昼間"""
        from zoneinfo import ZoneInfo
        mock_dt.now.return_value = datetime(2026, 3, 6, 12, 0, tzinfo=ZoneInfo("Asia/Tokyo"))
        assert is_night_mode(23, 5) is False
    ...
    def test_no_wrap(self, mock_dt) -> None:
        ...
        from zoneinfo import ZoneInfo
        mock_dt.now.return_value = datetime(2026, 3, 6, 3, 0, tzinfo=ZoneInfo("Asia/Tokyo"))
