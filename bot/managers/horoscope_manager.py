...
# AIモードのプロンプトテンプレート
# {ranking_lines} に事前決定済みのランキング順（順位＋星座名）が入る
HOROSCOPE_PROMPT_TEMPLATE = """
【条件】
- 本日の日付: {today}
- 以下のランキング順序は確定済みです。この順序を変更しないでください。
- 各星座に「⭐️」で評価（1位=5.0〜12位=0.5、0.5刻み）を付けてください。順位が高いほど高評価にしてください。
- 各星座にキャラクターの口調で今日の運勢コメント（15〜20文字程度）を付けてください。
- 1位は🥇️、2位は🥈️、3位は🥉️を付けてください。4位以降は数字のみ。
...
🥇️ [星座名(絵文字付き)] ⭐️[評価] [運勢コメント]
🥈️ [星座名(絵文字付き)] ⭐️[評価] [運勢コメント]
🥉️ [星座名(絵文字付き)] ⭐️[評価] [運勢コメント]
4位 [星座名(絵文字付き)] ⭐️[評価] [運勢コメント]
5位 [星座名(絵文字付き)] ⭐️[評価] [運勢コメント]
6位 [星座名(絵文字付き)] ⭐️[評価] [運勢コメント]
7位 [星座名(絵文字付き)] ⭐️[評価] [運勢コメント]
8位 [星座名(絵文字付き)] ⭐️[評価] [運勢コメント]
9位 [星座名(絵文字付き)] ⭐️[評価] [運勢コメント]
10位 [星座名(絵文字付き)] ⭐️[評価] [運勢コメント]
11位 [星座名(絵文字付き)] ⭐️[評価] [運勢コメント]
12位 [星座名(絵文字付き)] ⭐️[評価] [運勢コメント]
...
class HoroscopeManager:
    ...
    @staticmethod
    def _generate_ranking_order(date_str: str) -> list[tuple[str, str]]:
        """日付ベースのシードで星座ランキング順序を決定する。

        MD5ハッシュを使い、同じ日は同じ順序になる。
        """
        seed_str = date_str.replace("-", "")
        seed = int(hashlib.md5(seed_str.encode()).hexdigest(), 16)
        rng = random.Random(seed)
    ...
    async def _generate_ai_horoscope(self, date_str: str) -> str:
        ...
        ランキング順序はコードで事前決定し、AIにはコメント生成のみを任せる。
